## 2022-07-28 Kaining Hu
autoDESeq2.vsalmon <- function(I,C,n){
library(DESeq2)
inputfilename <- I
...
dds <- DESeqDataSetFromTximport(I,
                                   colData = Congroup,
                                   design = ~ Group + Batch)
dds <- DESeq(dds)
write.table(counts(dds,normalized=T),"normalizedallCDS.txt", sep="\t", col.names = NA)
write.table(counts(dds,normalized=F),"Raw_allCDS.txt", sep="\t", col.names = NA)
...
write.table(cor(I$counts),"corAllcds.txt",sep="\t", col.names = NA)
