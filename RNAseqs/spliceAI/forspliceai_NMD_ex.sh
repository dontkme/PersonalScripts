...
spliceA=grch38
for file in $(ls Test3_*NMD_ex*chr*.vcf)
do
...
outfile="Test3_humanunion_fixbug_rmNMD_GTF.NMD_exAS968.intron_region2.gnomad.${chr}.spliceai.D${spliceD}.vcf"
#echo $outfile
echo "#!/bin/bash" >${file}.sbatch
echo "#SBATCH --job-name=NMD_ex_spliceai.${chr}.D${spliceD}">>${file}.sbatch
echo "#SBATCH --output=NMD_ex_spliceai.${chr}.D${spliceD}.out">>${file}.sbatch
echo "#SBATCH --error=NMD_ex_spliceai.${chr}.D${spliceD}.err">>${file}.sbatch
