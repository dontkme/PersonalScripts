...
or die("[-]Error in command line arguments
  Usage: perl GetspliceAIresCounts.pl [options] -g <in.vcf.gz> <input region file>
    ...
    [-c float|Cutoff of delta Score.[0-1] Default: 0.5]
    [-d int|Filter snp distence to boundaries. Dfault: 5]
    Note:  Get spliceai results count (GnomAD version) v0.130 2022/02/09.\n");
...
while(defined(our $seq = <>)){
  ...
  if ($seq =~ m/^#.*/) {
    ...
    my $gene=$chrr[0];
    my $chr=$chrr[1];
    # $chr=~s/chr//; ### for diff chr header GnomAD version not need this line.
    my $pos1=$chrr[2];
    my $pos2=$chrr[3];
    my $PM=$chrr[4];
    # $pos2 =~ s/\s//g;
    ...
    while(defined(our $resline = <FILE>)){
        ...
        if ($resline =~ m/^#.*/) {
          ...
          # if ($resline=~ m/SpliceAI=(.*)$/){
            ...
            # my $spanno=$sepline[7];
            my $spanno=$sepline[10]; ## GnomeAD version is column 11
            $spanno =~ s/\s//g;
            # say $spanno;
