...
SMindex="/project/xczhang/Genomes/GENCODE43_salmonGenTrans_Index/GENCODE43_GenTrans_salmon_index"
...
#jobname="${prefix}.drop"
ncpu=10
jobname="${prefix2}.SMGT"
sbatchname="${prefix2}.SMGT.sbatch"
...
echo "$SMPATH/salmon quant -i $SMindex -l A -1 ${f1} -2 ${f2} -p $ncpu --validateMappings -o GTquants/${prefix}_quant" >>$sbatchname
