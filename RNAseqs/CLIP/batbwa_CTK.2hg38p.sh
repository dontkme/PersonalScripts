...
#refbwa="/project2/xczhang/genomes/GENCODE38_GRCh38.p13_ALL_BWA_index/GRCh38.p13.genome.fa"
refbwa="/home/kaininghu/xczhang/genomes/GENCODE38_GRCh38_BWA_index/GRCh38.primary_assembly.genome.fa"
...
echo "#SBATCH --mem=64G" >>$sbatchname
...
echo "samtools sort -@ $ncpu -o $f.sort.bam $f.bam" >>$sbatchname
...
echo "perl ${ctkpath}/bed2annotation.pl -dbkey $dbkey -ss -big -region -v -summary $f.tag.uniq.annot.summary.txt $f.tag.uniq.rgb.bed $f.tag.uniq.annot.txt">>$sbatchname
...
echo "perl ${ctkpath}/bed2annotation.pl -dbkey $dbkey -ss -big -region -v -summary $f.tag.uniq.peak.annot.summary.txt $f.tag.uniq.peak.bed $f.tag.uniq.peak.annot.txt">>$sbatchname
...
echo "perl ${ctkpath}/tag2peak.pl -big -ss -v --valley-seeking -p 0.05 --valley-depth 0.9 --dbkey $dbkey --multi-test $f.tag.uniq.rgb.bed $f.tag.uniq.peak.sig.bed --out-boundary $f.tag.uniq.peak.sig.boundary.bed --out-half-PH $f.tag.uniq.peak.sig.halfPH.bed">>$sbatchname
