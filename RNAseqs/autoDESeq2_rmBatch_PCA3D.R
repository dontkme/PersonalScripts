...
write.table(plotPCA(d,intgroup=G,ntop=n,returnData=T),file=txtname,sep="\t")
...
autoPCA3D <- function(i,n,d,G){
  ...
  txtname <- paste(compicname,".txt")
  write.table(plotPCA3D(d,intgroup=G,ntop=n,returnData=T),file=txtname,sep="\t")
...
assay(vsd) <- mat
write.table(assay(vsd),"vsd_rm_batch.txt",sep="\t")
...
write.table(res,comname,sep="\t",col.names=T)
