...
or die("[-]Error in command line arguments
  Usage: perl GetTransIDsfromGTF [options] <-g string|GTF annoation file> <input FASTA file(s)>
    options:
...
print "Feature: $feature\n";
    ...
    our $faheadid="test";
    our $annotcount=0;
    our (%key2Chr,%key2type, %key2startpos, %key2endpos, %key2PM,%key2restwords,%key2line,%key2transid,%key2geneid,%key2exonid,%SEexonid);
...
open INPUTLIST, "< $inputlist" or die ("[-] Error: Can't open list file: $inputlist.\n");
open OUTTRANSID, "> $opfn.outtransid.txt" or die ("[-] Error: Can't open or creat $opfn.outtransid.txt\n");
open OUTSETRANSID, "> $opfn.outSEtransid.txt" or die ("[-] Error: Can't open or creat $opfn.outSEtransid.txt\n");
open OUTUSTRANSID, "> $opfn.outUStransid.txt" or die ("[-] Error: Can't open or creat $opfn.outUStransid.txt\n");
open OUTDSTRANSID, "> $opfn.outDStransid.txt" or die ("[-] Error: Can't open or creat $opfn.outDStransid.txt\n");
open OUTUSDSTRANSID, "> $opfn.outUSDStransid.txt" or die ("[-] Error: Can't open or creat $opfn.outUSDStransid.txt\n");
        ...
        foreach my $keypos3(@sortkey){
            if ($keypos3 =~ m/$SEpos/){
                my $tmptransid = $key2transid{$keypos3};
                  $SEexonid{$tmptransid} = $key2exonid{$keypos3};
