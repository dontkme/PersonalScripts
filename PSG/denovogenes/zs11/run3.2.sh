...
for j in Bnazs11 Brapa3 BolHDEM BniB Tpa Ath11
do 
#sed '1,5d' Bnafr.${i}.dna.psl|sort -nk 1,1 |awk '{if($1/$11>=0.2){a[$10]++;b[$10]=$1/$11;c[$1]="'$i'_"$14}}END{for(i in a){print i,a[i],b[i],c[$1]}}'
sed '1,5d' Bnazs11.${j}.dna.psl|sort -nk 1,1 |awk -F" " '{if($1/$11>=0.2){a[$10]++;b[$10]=$1/$11;c[$1]="'$j'"}}END{for(i in a){print i,a[i],b[i],c[$1]}}'
#sed '1,5d' Bnazs11.${j}.dna.psl|sort -nk 1,1 |awk '{if($1/$11>=0.2){a[$10]++;b[$10]=$1/$11;c[$1]=$14}}END{for(i in a){print i,a[i],b[i],c[$1]}}'
done |awk '{print $4"_"$1,$2,$3}' |awk '{if(a[$1]<$2){a[$1]=$2}if(b[$1]<$3){b[$1]=$3}}END{for(i in a){print i,a[i],b[i]}}' |sed 's/_/ /' |awk '{if($3>1){s[$2]=1}else{s[$2]=s[$2]+0}if($1=="Bnazs11"){a1[$2]=$4}if($1=="Brapa3"){a2[$2]=$4}if($1=="BolHDEM"){a3[$2]=$4}if($1=="BniB"){a4[$2]=$4}if($1=="Tpa"){a5[$2]=$4}if($1=="Ath11"){a6[$2]=$4}}END{for(i in s){if(s[i]==0){print i,a1[i]+0,a2[i]+0,a3[i]+0,a4[i]+0,a5[i]+0,a6[i]+0}}}' >Bnazs11.denovo.dna.status
